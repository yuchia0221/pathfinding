...
function setChild(current_node, boardTwoD, visitedList) {
	...
	if (current_node.column < boardWidth - 1) {
		if (
			boardTwoD[current_node.row][current_node.column + 1].status ===
				"unvisited" ||
			boardTwoD[current_node.row][current_node.column + 1].status ===
				"target"
		) {
			current_node.child.push(
				boardTwoD[current_node.row][current_node.column + 1]
			);
			visitedList.push(
				boardTwoD[current_node.row][current_node.column + 1]
			);
			changeStatus(current_node.row, current_node.column + 1);
			boardTwoD[current_node.row][
				current_node.column + 1
			].father = current_node;
		}
	}
	...
	if (current_node.row > 0) {
		if (
			boardTwoD[current_node.row - 1][current_node.column].status ===
				"unvisited" ||
			boardTwoD[current_node.row - 1][current_node.column].status ===
				"target"
		) {
			current_node.child.push(
				boardTwoD[current_node.row - 1][current_node.column]
			);
			visitedList.push(
				boardTwoD[current_node.row - 1][current_node.column]
			);
			changeStatus(current_node.row - 1, current_node.column);
			boardTwoD[current_node.row - 1][
				current_node.column
			].father = current_node;
		}
	}
